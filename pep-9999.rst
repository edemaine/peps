...
well as generator expressions and arguments, to allow unpacking
notation (``*`` and ``**``) at the start of the expression.  For
example, ``[*it for it in its]`` becomes short-hand for
``[x for it in its for x in it]``.  This notation makes it easy to
combine an unknown number of iterables (given by a ``for`` clause)
into one list or set or generator, or an unknown number of
...
makes it easy to combine a few iterables or dictionaries::
    ...
    f(*it1, *it2, *it3)  # pass in the concatenation of three iterables
...
iterables or dictionaries, via comprehensions::
    ...
    f(*it for it in its) # pass in the concatenation of iterables in 'its'
...
The same analogies holds for the set, dictionary, generator, and
argument versions.
...
The rules for arguments currently refer to ``starred_expression``,
which would be modified to allow ``for`` clauses::
...
repeated elements/keys replace earlier instances.)  Similarly,
``f(*expr for x in it)`` works as if the expressions were listed via
``f(*expr1, *expr2, ...)``.
...
Another possible interpretation of ``f(*it for it in its)`` is that it
calls ``f`` with a single argument equal to the new generator
expression ``(*it for it in its)``.  PEP 289 [#PEP-289] in fact says
that the parentheses around a generator are optional in a
